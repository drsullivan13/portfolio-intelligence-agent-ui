...
export default function TickerView() {
  ...
  const tickerEvents = mockEvents.filter(e => e.ticker === symbol);
  ...
  const newsCount = tickerEvents.filter(e => e.event_type === 'NEWS').length;
  const filingCount = tickerEvents.filter(e => e.event_type === 'SEC_FILING').length;
  const avgSentiment = tickerEvents.reduce((acc, curr) => acc + (curr.sentiment_score || 0), 0) / (tickerEvents.length || 1);
        ...
        <div className="space-y-6">
           <Card>
             ...
             <CardContent className="space-y-4">
               <div>
                 <div className="text-2xl font-bold">{tickerEvents.length}</div>
